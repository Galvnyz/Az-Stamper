...
async function refreshActivity() {
  ...
  try {
    ...
    var kql = [
      ...
      '| extend ResourceId = coalesce(tostring(customDimensions.ResourceId), tostring(customDimensions.prop__ResourceId), "")',
      '| extend SubscriptionId = coalesce(tostring(customDimensions.SubscriptionId), tostring(customDimensions.prop__SubscriptionId), extract("/subscriptions/([^/]+)", 1, ResourceId))',
      '| extend ResourceType = coalesce(tostring(customDimensions.ResourceType), tostring(customDimensions.prop__ResourceType), extract("/providers/([^/]+/[^/]+)", 1, ResourceId))',
